...
class BulkAddModal(Modal, title="Bulk Add Collector Requirements"):
    ...
    async def _process(self, interaction: Interaction):
        ...
        for line in lines:
            ...
            added.append(f"**{ball.country}** - own atleast {amount:,} to get {special.name}")
...
class CollectorCog(commands.GroupCog, name="Collector"):
    ...
    async def list(
        ...
        title = f'{settings.bot_name} "{special.strip()}" Collector List' if special else f"{settings.bot_name} Collector List"
